...
(check-expect (next-game G3)
              (make-game
               (list (make-invader (+ 150 (* -10 INVADER-X-SPEED))
                                   (+ HEIGHT (* (abs -10) INVADER-Y-SPEED)) -10))
...
(define (next-game g)
  (handle-collisions (make-game (handle-invaders (game-invaders g))
...
;; ---------------------
;; Tank -> Tank
;; Moves the tank either left or right (dependent on dir) TANK-SPEED units.
;; ---------------------
(check-expect (move-tank T2) (make-tank (+ 50 TANK-SPEED) 1))
(check-expect (move-tank T3) (make-tank (- 50 TANK-SPEED) -1))
...
(define (move-tank t)
  (cond [(> (+ (tank-x t) (* TANK-SPEED (tank-dir t))) (- WIDTH (/ (image-width TANK) 2)))
        ...
        [(< (+ (tank-x t) (* TANK-SPEED (tank-dir t))) (+ 0 (/ (image-width TANK) 2)))
        (make-tank (+ 0 (/ (image-width TANK) 2)) (tank-dir t))]
...
;; ---------------------
(check-expect (handle-key G0 "shift") G0)
(check-expect (handle-key G0 "right") (make-game empty empty (make-tank CTR-Y 1)))
(check-expect (handle-key (make-game empty empty (make-tank CTR-Y -1)) "right")
              (make-game empty empty (make-tank CTR-Y 1)))
(check-expect (handle-key (make-game empty empty (make-tank CTR-Y 1)) "left")
              (make-game empty empty (make-tank CTR-Y -1)))
(check-expect (handle-key G0 "left") (make-game empty empty (make-tank CTR-Y -1)))
(check-expect (handle-key G0 " ") (make-game empty (list (make-missile CTR-Y TANK-Y)) T1))

;(define (handle-key g ke) g) ; stub

(define (handle-key g ke)
  (cond [(key=? ke "right") (move-right g)]
        [(key=? ke "left") (move-left g)]
...
;; ---------------------
;; Game -> Game
;; Sets the tank dir to 1, indicates the tank is moving to the right.
;; ---------------------
(check-expect (move-right G0) G0)
(check-expect (move-right (make-game empty empty (make-tank CTR-Y -1)))
              (make-game empty empty (make-tank CTR-Y 1)))
...
;; ---------------------
;; Game -> Game
;; Sets the tank dir to -1, indicates the tank is moving to the left.
;; ---------------------
(check-expect (move-left G0)
              (make-game empty empty (make-tank CTR-Y -1)))
(check-expect (move-left (make-game empty empty (make-tank CTR-Y -1)))
              (make-game empty empty (make-tank CTR-Y -1)))
...
;; ---------------------
;; Game -> Game
;; Moves the tank TANK-SPEED px to the left, (- TANK-X TANK-SPEED)
;; ---------------------
(check-expect (fire-missile G0) (make-game empty (list (make-missile CTR-Y TANK-Y)) T1))
